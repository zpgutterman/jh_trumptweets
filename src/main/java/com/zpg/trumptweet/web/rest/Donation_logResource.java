...
public class Donation_logResource {
	 ...
	 *
	 * @param donation_log
	 *            the donation_log to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new donation_log, or with status 400 (Bad Request) if the
	 *         donation_log has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/donation-logs")
	@Timed
	public ResponseEntity<Donation_log> createDonation_log(@Valid @RequestBody Donation_log donation_log)
			throws URISyntaxException {
		log.debug("REST request to save Donation_log : {}", donation_log);
		if (donation_log.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists",
					"A new donation_log cannot already have an ID")).body(null);
		}
	 ...
	 *
	 * @param donation_log
	 *            the donation_log to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         donation_log, or with status 400 (Bad Request) if the
	 *         donation_log is not valid, or with status 500 (Internal Server
	 *         Error) if the donation_log couldnt be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/donation-logs")
	@Timed
	public ResponseEntity<Donation_log> updateDonation_log(@Valid @RequestBody Donation_log donation_log)
			throws URISyntaxException {
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, donation_log.getId().toString())).body(result);
	}
	...
	/**
	 * GET /donation-month get the donation total for the current user for the
	 * current month
	 * 
	 * @param id
	 *            the id of the donation_log to retrieve
	...
	@Timed
	public ResponseEntity<List<Donation_log>> getPendingPayments() throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the donation_log to delete
